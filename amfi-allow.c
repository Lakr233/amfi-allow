// amfi-allow — let an ad-hoc signed binary carry Apple-private entitlements,
// without writing a single byte into anyone's __TEXT.
//
// Build (no Xcode.app, no Python, no LLDB):
//
//     clang -arch arm64e -O2 -framework CoreFoundation -framework Security \
//         -o amfi-allow amfi-allow.c
//     codesign --force --sign - amfi-allow
//
// ---------------------------------------------------------------------------
// Why this exists
// ---------------------------------------------------------------------------
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr,
            "usage:\n"
            "  amfi-allow status\n"
            "  sudo amfi-allow allow [--hold N] <binary> [<binary>...]\n"
            "  sudo amfi-allow off\n");
    return 2;
}
